...
public class JwtTokenProvider {
	...
	public boolean validateToken(String token) {
		try {
			...
			.setSigningKey(key)
			.build()
			.parseClaimsJws(token); //parseClaimsJw't' 조심
			
			return true;
		}catch (SecurityException | MalformedJwtException e) {
			// Security 라이브러리에 오류가 있거나, JSON의 포맷이 잘못된 형식의 JWT가 들어왔을 때
			// SignatureException이 포함 되어 있음.
			log.info("Invalid JWT Token", e);
		}catch (ExpiredJwtException e) {
			// 토큰의 유효기간이 만료된 경우 예외
			log.info("Expired JWT Token", e);
		}catch (UnsupportedJwtException e) {
			// jwt의 형식을 지키지 않은 경우 (Header.Payload.Signature의 형식이 지켜지지 않을 때)
	...
	private Claims parseClaims(String acessToken) {
		try {
